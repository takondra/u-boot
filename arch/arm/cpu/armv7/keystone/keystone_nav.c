...
struct qm_host_desc * qm_pop(u32 qnum)
{
	...
	uhd1 = uhd = readl(&qm_cfg->queue[qnum].ptr_size_thresh) & ~0xf;
	if (uhd)
...
static int netcp_rx_disable(void)
{
	...
	for (j = 0; j < netcp->rx_ch_num; j++) {
		if (!((v = readl(&netcp->rx_ch[j].cfg_a)) & CPDMA_CHAN_A_ENABLE))
			continue;

		writel(v | CPDMA_CHAN_A_TDOWN, &netcp->rx_ch[j].cfg_a);
		for (k = 0; k < TDOWN_TIMEOUT_COUNT; k++) {
			udelay(100);
			if (!((v = readl(&netcp->rx_ch[j].cfg_a)) &
			      CPDMA_CHAN_A_ENABLE))
				continue;
...
static int netcp_tx_disable(void)
{
	...
	for (j = 0; j < netcp->tx_ch_num; j++) {
		if (!((v = readl(&netcp->tx_ch[j].cfg_a)) & CPDMA_CHAN_A_ENABLE))
			continue;
...
static int _netcp_init(struct pktdma_cfg *netcp_cfg, struct rx_buff_desc *rx_buffers)
{
	u32 j, v;
	struct qm_host_desc *hd;
	u8* rx_ptr;
	...
	v = CPDMA_REG_VAL_MAKE_RX_FLOW_D(0, netcp->rx_free_q, 0, netcp->rx_free_q);
