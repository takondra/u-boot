...
static void mmc_reset_controller_fsm(struct hsmmc *mmc_base, u32 bit)
{
	...
	start = get_timer(0);
	/*
	 * CMD(DAT) lines reset procedures are slightly different for OMAP3
	 * and OMAP4(OMAP5).
	 * According to OMAP3 TRM:
	 * Set SRC(SRD) bit in MMCHS_SYSCTL register to 0x1 and wait until
	 * it returns to 0x0.
	 * According to OMAP4(OMAP5) TRMs, CMD(DATA) lines reset procedure
	...
	while (!(readl(&mmc_base->sysctl) & bit)) {
		if (get_timer(0) - start > MAX_RETRY_MS) {
			printf("%s: timedout waiting for sysctl %x to set\n",
				__func__, bit);
			return;
		}
	}
